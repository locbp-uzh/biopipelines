#!/usr/bin/env python3
"""
Process sequences through MMseqs2 LCF (LocalColabFold) server and generate MSA files.

This script reads a sequences CSV file, submits each sequence to the MMseqs2LCF server,
and generates A3M MSA files. Uses colabfold_search backend for richer MSAs combining
UniRef30 and ColabFoldDB (environmental sequences).
"""
...
def handle_server_error(server_dir):
    ...
    if random.random() < 0.05:
        ...
        try:
            ...
            if match:
                log(f"MMseqs2 LCF server resubmitted with job ID: {match.group(1)}")
            else:
                try:
...
def check_and_resubmit_server(server_dir):
    """Check if MMseqs2 LCF server is running, and resubmit if not."""
    ...
    # Check if GPU server is running and valid
    log("Checking GPU server...")
    server_running = False
    if server_is_valid(GPU_TIMESTAMP):
        log("MMseqs2 LCF GPU server is running and valid")
        server_running = True
    ...
    if not server_running:
        log("No valid MMseqs2 LCF server found, resubmitting server...")
        ...
        try:
            ...
            if match:
                ...
                log(f"MMseqs2 LCF server submitted with job ID: {server_job_id}")
...
def submit_sequence_to_server(sequence, sequence_id, client_script, output_path=None, timeout=1800, server_dir=None):
    """Submit a single sequence to MMseqs2 LCF server with specified output path."""
    try:
        # Build command
        cmd = [
            client_script,
        ...
        if output_path:
            ...
            log(f"Submitting sequence {sequence_id} to MMseqs2 LCF server (output: {output_path})")
        else:
            log(f"Submitting sequence {sequence_id} to MMseqs2 LCF server")

        result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)

        if result.returncode != 0:
            log(f"ERROR: MMseqs2 LCF server failed for sequence {sequence_id}")
...
def read_a3m_sequences(a3m_file):
...
def process_a3m_output(a3m_file, sequence_id, output_a3m_file, mask_positions=None):
    """Process A3M file with optional masking and save."""
    try:
        sequences = read_a3m_sequences(a3m_file)
        ...
        if mask_positions:
            ...
            # Re-write the A3M file with masked sequences
            # Read headers from original
            headers = []
            with open(a3m_file, 'r') as f:
                for line in f:
                    if line.startswith('>'):
            ...
            log(f"Saved masked A3M to: {output_a3m_file}")
        else:
            # No masking - just copy the file
            if a3m_file != output_a3m_file:
                import shutil
                shutil.copy(a3m_file, output_a3m_file)

        # Get query sequence (first sequence in A3M)
        query_sequence = sequences[0] if sequences else ""

        # Return single summary row pointing to the A3M file
        return [{
            'id': f"{sequence_id}_msa",
            'sequence_id': sequence_id,
            'sequence': query_sequence,
            'msa_file': output_a3m_file
        }]
...
def main():
    parser = argparse.ArgumentParser(description='Process sequences through MMseqs2 LCF server')
    ...
    parser.add_argument('client_script', help='Path to MMseqs2 LCF client script')
    parser.add_argument('--server_dir', required=True,
                       help='Path to MMseqs2 LCF server directory for timestamp files')
    ...
    for _, row in sequences_df.iterrows():
        ...
        # Save individual MSA file with sequence ID name (A3M format for LCF)
        output_dir = os.path.dirname(args.output_msa_csv)
        individual_msa_file = os.path.join(output_dir, f"{sequence_id}.a3m")
        ...
        if os.path.exists(individual_msa_file):
            ...
            try:
                sequences = read_a3m_sequences(individual_msa_file)
                if sequences:
                    ...
                    log(f"WARNING: Existing MSA file is empty, will reprocess")
        ...
        if mask_positions:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.a3m', delete=False) as temp_file:
                temp_output = temp_file.name

            result_file = submit_sequence_to_server(
                sequence, sequence_id, args.client_script, temp_output,
            ...
            msa_rows = process_a3m_output(result_file, sequence_id, individual_msa_file, mask_positions)
            ...
            result_file = submit_sequence_to_server(
                sequence, sequence_id, args.client_script, individual_msa_file,
                server_dir=args.server_dir
            )
            ...
            msa_rows = process_a3m_output(individual_msa_file, sequence_id, individual_msa_file, None)
