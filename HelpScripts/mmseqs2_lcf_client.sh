...
# MMseqs2 LCF client working directories - must match server script structure
USER=${USER:-$(whoami)}
...
usage() {
    echo "Usage: $0 -s <sequence> [-f <fasta_file>] [-o <output_file>]"
    echo ""
    echo "Options:"
    echo "  -s, --sequence   Protein sequence to search"
    echo "  -f, --fasta      Input FASTA file (alternative to -s)"
    echo "  -o, --output     Output path for A3M result"
    echo "  -u, --status     Check server status"
}
...
if [[ "$STATUS" -eq 1 ]]; then
  ...
  check_server_timestamp() {
    ...
    if [[ ! -f "$timestamp_file" ]]; then
      log "MMseqs2 LCF $server_type server: NOT RUNNING (no timestamp file)"
      return 1
    fi
    ...
    if [[ $time_diff -lt $max_age_seconds ]]; then
      log "MMseqs2 LCF $server_type server: RUNNING (started at $server_time, ${hours_old}h ${minutes_old}m ago)"
      return 0
    else
      log "MMseqs2 LCF $server_type server: EXPIRED (started at $server_time, ${hours_old}h ${minutes_old}m ago, max age: ${MAX_AGE_HOURS}h)"
      return 1
    fi
  }
  ...
  if [[ "$gpu_running" = false ]]; then
    log "No valid MMseqs2 LCF server is running"
  fi
...
while [[ $retry_count -lt $MAX_RETRIES ]]; do
  ...
  log "Submitting LCF MSA job: $job_id"
  job_file="$JOB_QUEUE_DIR/$job_id.job"

  # Write out all needed params (LCF always outputs A3M format)
  {
    echo "job_id=$job_id"
