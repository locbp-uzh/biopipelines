...
- [ ] `docs/tool/<category>.md`: added a `**Tags**:` line to that section, from the closed vocabulary in `docs/tool_tags.md`. At least one ACTION tag is required, plus every SUBJECT, READOUT and CAPABILITY term that is true **of this wrapper** — not of the upstream project, whose extra capabilities are feature requests, not tags. `tests/test_tool_tags.py` also fails if the section's prose or parameters argue for a capability tag (`covalent`, `symmetry`, `all-atom`, `sequence-only`, …) the line omits; if the keyword genuinely means something else there, add the tool to `tool_tags.LINT_EXEMPT` with the reason rather than muting the test.
...
- [ ] Regenerated the agent-facing index: `python skills/biopipelines/build_tool_index.py`. `tests/test_skill_tool_index.py` compares it byte-for-byte against the docs, so the new tool must appear there too — it is what an agent reads to discover the tool exists.
