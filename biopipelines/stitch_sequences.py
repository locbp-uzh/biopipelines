        ...
        if isinstance(source, str):
            ...
            if source.map_table:
                return {
                    ...
                    "sequence_ids": list(source.ids_expanded) or []
                }
            elif source.files:
                return {
                    "type": "tool_output",
                    "sequences_file": source.files_expanded[0],
                    "source_name": "DataStream",
                    "sequence_ids": list(source.ids_expanded) or []
                }
            else:
            ...
            result = {
                ...
                "sequence_ids": list(source.streams.sequences.ids_expanded) if source.streams.sequences else []
            }
            ...
            if source.streams.structures and len(source.streams.structures) > 0:
                result["structure_files"] = list(source.streams.structures.files_expanded)
                result["structure_ids"] = list(source.streams.structures.ids_expanded) or []
...
"""
    ...
    def _predict_output_sequence_ids(self) -> List[str]:
        ...
        if isinstance(self.template, str):
            ...
            template_ids = list(self.template_stream.ids_expanded) or ["seq"]
        else:
            template_ids = ["seq"]
    ...
    def _predict_matched_sequence_ids(self, template_ids: List[str]) -> List[str]:
        ...
        for options in self.substitutions.values():
            if isinstance(options, StandardizedOutput):
                sub_ids_list.append(list(options.streams.sequences.ids_expanded) if options.streams.sequences else [])
            elif isinstance(options, DataStream):
                sub_ids_list.append(list(options.ids_expanded) or [])

        # Get sequence_ids from each indel source
        indel_ids_list = []
        for options in self.indels.values():
            if isinstance(options, StandardizedOutput):
                indel_ids_list.append(list(options.streams.sequences.ids_expanded) if options.streams.sequences else [])
            elif isinstance(options, DataStream):
                indel_ids_list.append(list(options.ids_expanded) or [])
