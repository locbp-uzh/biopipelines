...
original_analysis = pipeline.add(MergeDatasheets(datasheets=[original_open.o.datasheets.affinity,
                                                             original_close.o.datasheets.affinity],
...
"""
boltz_apo_first10 = pipeline.add(Boltz2(proteins=first_10_sequences.output))
boltz_holo_open_first10 = pipeline.add(Boltz2(proteins=first_10_sequences.output,
                                ligands=OPEN_SMILES_R,
                                msas=boltz_apo_first10.output,
                                affinity=True))
boltz_holo_close_first10 = pipeline.add(Boltz2(proteins=first_10_sequences.output,
                                ligands=CLOSE_SMILES_R,
                                msas=boltz_apo_first10.output,
                                affinity=True))
open_chlorine_aspartate_distance_first10 = pipeline.add(ResidueAtomDistance(input=boltz_holo_open_first10.output,
                                                                    residue='D in IHDWG',
                                                                    atom='LIG.Cl',
                                                                    metric_name='open_chlorine_distance'))
close_chlorine_aspartate_distance_first10 = pipeline.add(ResidueAtomDistance(input=boltz_holo_close_first10.output,
                                                                    residue='D in IHDWG',
                                                                    atom='LIG.Cl',
                                                                    metric_name='close_chlorine_distance'))
analysis_first10 = pipeline.add(MergeDatasheets(datasheets=[boltz_holo_open_first10.output.datasheets.affinity,
                                                    boltz_holo_close_first10.output.datasheets.affinity,
                                                    open_chlorine_aspartate_distance_first10.output.datasheets.analysis,
                                                    close_chlorine_aspartate_distance_first10.output.datasheets.analysis],
                                        prefixes=["open_","close_","",""],
                                        calculate = {"affinity_delta":"open_affinity_pred_value-close_affinity_pred_value"} ))
...
profiler = pipeline.add(MutationProfiler(original=original_open.output,
...
for mode in ["single_point","weighted_random","hotspot_focused","top_mutations"]:
    ...
    analysis = pipeline.add(MergeDatasheets(datasheets=[boltz_holo_open.output.datasheets.affinity,
                                            ...
                                            prefixes=["open_","close_","",""],
                                            calculate = {"affinity_delta":"open_affinity_pred_value-close_affinity_pred_value"} ))
    filtered_analysis = pipeline.add(Filter(data=analysis.output,
...
pipeline.add(ConcatenateDatasheets([x.output.datasheets.merged for x in all_analysis]))
pipeline.add(ExtractMetrics(datasheets=[x.output.datasheets.merged for x in all_analysis],
