...
with Pipeline(project="RefactorTest",
    ...
    filtered = Panda(
        table=boltz1.tables.confidence,
        operations=[
            Panda.filter("confidence_score > 0.5")
        ]
    )
    ...
    ranked = Panda(
        table=boltz1.tables.confidence,
        operations=[
            Panda.rank(by="confidence_score", ascending=False)
        ]
    )
    ...
    deduped = Panda(
        table=boltz1.tables.confidence,
        operations=[
            Panda.drop_duplicates(subset="id", keep="first")
        ]
    )
    ...
    calculated = Panda(
        table=boltz1.tables.confidence,
        operations=[
            Panda.calculate({"conf_scaled": "confidence_score * 100"})
        ]
    )
    ...
    complex_result = Panda(
        ...
        operations=[
            Panda.concat(fill="", add_source=True),
            Panda.filter("confidence_score > 0.3"),
            Panda.sort("confidence_score", ascending=False),
            Panda.rank(by="confidence_score", ascending=False),
            Panda.head(5)
        ]
    ...
    grouped = Panda(
        ...
        operations=[
            Panda.concat(fill="", add_source=True),
            Panda.groupby("source_table", {"confidence_score": "mean"})
        ]
    )
