...
with Pipeline(pipeline_name="Examples",
              job_name="LigandMPNN-MutationComposer-Cycle-Enantiomers",
    ...
    best_RR = LoadOutput('/shares/locbp.chem.uzh/public/BioPipelines/Boltz/HT7_Cy7_C_RR_001/ToolOutputs/1_Boltz2_output.json')
    ...
    for CYCLE in range(NUM_CYCLES):
        ...
        lmpnn_R = LigandMPNN(structures=best_R, #this is equivalent to boltz2
                                        ligand="LIG", #in ligand mpnn you should always specify the ligand name, which is LIG if from Boltz
                                        num_sequences=1000,
                                        batch_size=25, 
                                        redesigned=mutation_range, 
                                        design_within=4)
        lmpnn_S = LigandMPNN(structures=best_S, #this is equivalent to boltz2
                                        ligand="LIG", #in ligand mpnn you should always specify the ligand name, which is LIG if from Boltz
                                        num_sequences=1000,
                                        batch_size=25, 
                                        redesigned=mutation_range,
        ...
        profiler_R = MutationProfiler(original=best_R,
                                                mutants=lmpnn_R)
        ...
        boltz_holo_R = Boltz2(proteins=unique_new_sequences,
                                        ligands=best_R,
                                        msas=boltz_apo,
                                        affinity=True)
        boltz_holo_RR = Boltz2(proteins=unique_new_sequences,
                                        ligands=best_R,
                                        msas=boltz_apo,
                                        affinity=True)
        boltz_holo_S = Boltz2(proteins=unique_new_sequences,
                                        ligands=best_S,
                                        msas=boltz_apo,
                                        affinity=True)
        boltz_holo_SS = Boltz2(proteins=unique_new_sequences,
                                        ligands=best_SS,
                                        msas=boltz_apo,
                                        affinity=True)
        
        R_chlorine_aspartate_distance = ResidueAtomDistance(structures=boltz_holo_R,
                                                                            residue='D in IHDWG',
                                                                            atom='LIG.Cl',
                                                                            metric_name='chlorine_distance')
        S_chlorine_aspartate_distance = ResidueAtomDistance(structures=boltz_holo_S,
                                                                            residue='D in IHDWG',
                                                                            atom='LIG.Cl',
                                                                            metric_name='chlorine_distance')
        R_cap_aspartate_distance = ResidueAtomDistance(structures=boltz_holo_R,
                                                                    residue='D in IHDWG',
                                                                    atom='LIG.N88',
                                                                    metric_name='cap_distance')
        S_cap_aspartate_distance = ResidueAtomDistance(structures=boltz_holo_S,
                                                                    residue='D in IHDWG',
                                                                    atom='LIG.N88',
                                                                    metric_name='cap_distance')
