...
class RFDAA_PrepareLigand(BaseConfig):
    ...
    RFdiffusion-AllAtom requires at least some protein atoms in the input PDB, even for
    de novo protein design with ligands. This tool adds a simple peptide (1PEF) to ligand
    structures, which RFdiffusion-AllAtom will then ignore during diffusion.
    """
    ...
    def __init__(self, ligand: Union[ToolOutput, StandardizedOutput], **kwargs):
        ...
        Args:
            ligand: Tool output containing ligand structure(s)
            **kwargs: Additional parameters
        """
        self.ligand = ligand
        self.ligand_is_tool_output = False
        self.ligand_source_file = None

        # Handle tool output
        if isinstance(ligand, (ToolOutput, StandardizedOutput)):
            self.ligand_is_tool_output = True
            if isinstance(ligand, StandardizedOutput):
                # Get first structure from StandardizedOutput
                if hasattr(ligand, 'structures') and ligand.structures:
                    self.ligand_source_file = ligand.structures[0]
                    if len(ligand.structures) > 1:
                        print(f"Warning: Multiple structures found in input, using first one: {os.path.basename(self.ligand_source_file)}")
                else:
                    raise ValueError("No structures found in StandardizedOutput for ligand parameter")
            else:  # ToolOutput
                # Get first structure from ToolOutput
                structures = ligand.get_output_files("structures")
                if structures:
                    self.ligand_source_file = structures[0]
                    if len(structures) > 1:
                        print(f"Warning: Multiple structures found in tool output, using first one: {os.path.basename(self.ligand_source_file)}")
                    # Add dependency
                    self.dependencies.append(ligand.config)
                else:
                    raise ValueError("No structures found in ToolOutput for ligand parameter")
        else:
            raise ValueError("ligand parameter must be ToolOutput or StandardizedOutput")
    ...
    def validate_params(self):
        """Validate parameters."""
        if not self.ligand_is_tool_output:
            raise ValueError("ligand must be a ToolOutput or StandardizedOutput")

    def configure_inputs(self, pipeline_folders: Dict[str, str]):
        """Configure inputs."""
        self.folders = pipeline_folders
        self.input_sources["ligand"] = self.ligand_source_file

    def get_config_display(self) -> List[str]:
        """Get configuration display."""
        config_lines = super().get_config_display()
        config_lines.append(f"LIGAND_SOURCE: {os.path.basename(self.ligand_source_file)}")
        return config_lines
    ...
    def generate_script(self, script_path: str) -> str:
        ...
        # Output files
        prepared_pdb = os.path.join(output_folder, "prepared_ligand.pdb")
        structures_csv = os.path.join(output_folder, "structures.csv")

        # Helper script path
        helper_script = os.path.join(self.folders["HelpScripts"], "pipe_rfdaa_prepare_ligand.py")

        script_content = f"""#!/bin/bash
# RFDAA_PrepareLigand execution script
# Generated by BioPipelines pipeline system

{self.generate_completion_check_header()}

echo "Preparing ligand structure for RFdiffusion-AllAtom"
echo "Input ligand: {self.ligand_source_file}"
echo "Output: {prepared_pdb}"

# Run preparation script
python "{helper_script}" \\
  --ligand_pdb "{self.ligand_source_file}" \\
...
"""
    ...
    def get_output_files(self) -> Dict[str, List[str]]:
        ...
        prepared_pdb = os.path.join(self.output_folder, "prepared_ligand.pdb")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        base_dict.update({
            "tool_params": {
                "ligand_source": self.ligand_source_file
            }
        })
        return base_dict