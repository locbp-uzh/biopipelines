...
class DynamicBind(BaseConfig):
    ...
    def __init__(self,
        ...
        Args:
            ...
            ligands: Input ligands - SMILES string, ToolOutput, or StandardizedOutput
                    ...
                    If ToolOutput/StandardizedOutput: uses compounds table (must have 'smiles' column)
    ...
    def _initialize_file_paths(self):
        ...
        self.output_compounds_table = None
    ...
    def _setup_file_paths(self):
        ...
        # Core output files
        self.main_table = os.path.join(self.output_folder, "dynamicbind_results.csv")
        self.output_compounds_table = os.path.join(self.output_folder, "compounds.csv")
        ...
        if hasattr(self, 'folders') and self.folders:
            ...
            self.input_ligands_file = self.output_compounds_table
        else:
    ...
    def generate_script(self, script_path: str) -> str:
        ...
        # DynamicBind manages its own environments (dynamicbind + relax) in generate_script_run_dynamicbind
    ...
    def generate_script_run_dynamicbind(self) -> str:
        ...
        if self.ligands_is_smiles:
            ...
            compounds_df.to_csv(self.output_compounds_table, index=False)
        elif self.ligands_is_tool_output and "ligands" in self.input_sources:
            # Read tool output compounds table and create new CSV with 'ligand' column
...
python {self.folders["HelpScripts"]}/pipe_dynamicbind_prepare_ligands.py "{self.input_sources["ligands"]}" "{self.output_compounds_table}"

"""

        # Get Python paths from both environments (dynamicbind at index 0, relax at index 1)
        script += """# Get Python paths from both environments
"""
...
"""
            script += self.activate_environment(index=0)
            script += f"""python {self.inference_py_file} {protein_file} {self.input_ligands_file} {' '.join(protein_args)} --python $DYNAMICBIND_PYTHON --relax_python $RELAX_PYTHON
"""
    ...
    def generate_script_create_table(self) -> str:
        ...
        # We'll create our standardized table from it
        return f"""echo "Creating results table"

# Parse DynamicBind outputs into standardized table
python {self.folders["HelpScripts"]}/pipe_dynamicbind_table.py "{db_job_folder}" "{self.main_table}"
...
"""
    ...
    def get_output_files(self) -> Dict[str, List[str]]:
        ...
        # We'll return the output folder for now and let the table determine actual files
        structures = []
        structure_ids = []
        ...
        return {
            ...
            "compounds": [self.output_compounds_table],
            "compound_ids": [],
            "sequences": [],
            "sequence_ids": [],
            "tables": tables,
