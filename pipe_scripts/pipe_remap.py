...
def main():
    ...
    for stream in streams:
        ...
        target_folder = stream.get("target_folder") or output_folder

        if not old_files:
            print(f"  Stream '{stream_name}': no files (value-based), skipping symlinks")
            continue

        print(f"  Stream '{stream_name}': creating links for {len(old_files)} files in {target_folder}")
        for old_id, old_file in zip(old_ids, old_files):
            new_id = id_mapping.get(old_id, old_id)
            ext = os.path.splitext(old_file)[1]
            new_file = os.path.join(target_folder, f"{new_id}{ext}")
    ...
    for stream in streams:
        ...
        if not map_table_path:
            continue
        stream_name = stream.get("name", "unknown")
        output_map = stream.get("target_map_table") or os.path.join(output_folder, f"{stream_name}_map.csv")
        print(f"  Remapping map_table: {stream_name} -> {output_map}")
    ...
    for table in tables:
        ...
        if not table_path:
            continue
        output_table = table.get("target_path") or os.path.join(output_folder, f"{table_name}.csv")
        print(f"  Remapping table: {table_name} -> {output_table}")
