...
def stitch_sequences_from_config(config_data: Dict[str, Any]) -> None:
    ...
    if all_positions_fixed and all_subs_raw and all_indel_positions_fixed and all_indels_raw:
        ...
        results = stitch_matched_sequences(template_sequences, substitutions, indels)
...
def stitch_matched_sequences(
    ...
    # Group substitution sequences by template ID and load table positions if needed
    sub_grouped = []
    sub_position_maps = []
    for sub in substitutions:
    ...
    # Group indel sequences by template ID and load table positions if needed
    indel_grouped = []
    indel_position_maps = []
    for indel in indels:
        grouped = group_sequences_by_template(indel['sequences'], all_template_ids)
    ...
    for template_id, template_seq in sorted(template_sequences.items()):
        ...
        # Get substitution positions for this template
        ...
        for pos_key, pos_map in sub_position_maps:
            if pos_key['type'] == 'fixed':
                ...
                match_result = get_mapped_ids([template_id], pos_map_ids, unique=True)
        ...
        # Get indel positions for this template
