...
class PlotBuilder:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.tables_folder = config.get("tables_folder") or self.output_folder
        self.plot_filenames = config.get("plot_filenames", [])

        # Track metadata for all plots
        self.metadata: List[Dict[str, Any]] = []

    def _plot_data_csv_path(self, output_path: str) -> str:
        """Return the path for the CSV data sidecar of a plot.

        Each plot is a self-contained <name>.png + <name>.csv pair living
        together in the plots/ stream folder.
        """
        stem = os.path.splitext(os.path.basename(output_path))[0]
        return os.path.join(os.path.dirname(output_path), f"{stem}.csv")
    ...
    def execute_heatmap(self, op: Dict[str, Any], output_path: str):
        ...
        if columns:
            ...
            # Detect categorical (non-numeric) value column
            is_categorical = not pd.api.types.is_numeric_dtype(df[value_col])

            if is_categorical:
                # Map categories to integers for plotting
                categories = sorted(df[value_col].dropna().unique().tolist())
                cat_to_int = {cat: i for i, cat in enumerate(categories)}
                df = df.copy()
                df[value_col + "_num"] = df[value_col].map(cat_to_int)
                pivot_df = df.pivot_table(index=y_col, columns=x_col,
                                          values=value_col + "_num", aggfunc='first')
                vmin, vmax = 0, len(categories) - 1
            else:
                # Create pivot table
                pivot_df = df.pivot_table(index=y_col, columns=x_col,
        ...
        if is_categorical:
            ...
            base_cmap = plt.cm.get_cmap(cmap, n_cats)
            colors = [base_cmap(i) for i in range(n_cats)]
            discrete_cmap = ListedColormap(colors)
            bounds = np.arange(-0.5, n_cats, 1)
        ...
        if len(x_labels) > max_ticks:
            ...
            x_tick_pos = np.arange(0, len(x_labels), step)
            ax.set_xticks(x_tick_pos)
            ax.set_xticklabels([x_labels[i] for i in x_tick_pos], rotation=45, ha='right', fontsize=9)
        else:
            ax.set_xticks(np.arange(len(x_labels)))
            ax.set_xticklabels(x_labels, rotation=45, ha='right', fontsize=9)

        if len(y_labels) > max_ticks:
            step = max(1, len(y_labels) // max_ticks)
            y_tick_pos = np.arange(0, len(y_labels), step)
            ax.set_yticks(y_tick_pos)
            ax.set_yticklabels([y_labels[i] for i in y_tick_pos], fontsize=9)
        else:
        ...
        # Add annotations
        if annotate:
            int_to_cat = {i: cat for i, cat in enumerate(categories)} if is_categorical else None
            for i in range(len(y_labels)):
                for j in range(len(x_labels)):
                    ...
                    if not np.isnan(value):
                        ...
                        label = int_to_cat[int(value)] if int_to_cat else f'{value:.2f}'
    ...
    def _write_metadata(self):
        ...
        metadata_path = os.path.join(self.tables_folder, "plot_metadata.csv")
        if self.metadata:
