...
def parse_arguments():
    ...
    parser.add_argument('--ligand-csv', required=False, default=None,
                        help='Path to ligand compounds.csv file (optional in protein-anything target-only mode)')
...
def create_design_spec(ligand_info, binder_min, binder_max, target_structure=None,
    ...
    # Add ligand entity only if ligand info was provided. In protein-anything
    # target-only mode the wrapper omits the ligand entirely.
...
def main():
    ...
    # Load ligand information (optional in protein-anything target-only mode,
    # where the wrapper passes --ligand-csv NONE).
    if args.ligand_csv and args.ligand_csv != "NONE":
        ...
        print("\nNo ligand provided (protein-anything target-only mode)")
        ligand_info = None
