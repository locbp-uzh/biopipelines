...
class PyMOLSessionBuilder:
    ...
    def execute_load_states(self, op: Dict[str, Any]):
        """
        Execute LoadStates operation - load all structures as states of one PyMOL object.

        Each structure file becomes a separate state in a single multi-state object,
        allowing scrubbing through them in PyMOL's movie player. After loading,
        all states are aligned to a reference (first loaded object) or to each
        other via intra_fit.

        Args:
            op: Operation dict with structures reference and object name
        """
        structures_ref = op.get("structures")
        obj_name = op.get("name", "states")

        if not structures_ref:
            raise ValueError("LoadStates operation requires structures")

        structures = self._resolve_structures(structures_ref)
        print(f"LoadStates: Loading {len(structures)} structures as states of '{obj_name}'")
        ...
        for i, struct in enumerate(structures):
            ...
            try:
                # Load into the same object; PyMOL auto-creates a new state
                cmd.load(struct_path, obj_name)
                loaded_count += 1
                print(f"  State {loaded_count}: {struct_id}")
            except Exception as e:
                print(f"  Error loading {struct_path}: {e}")

        if loaded_count == 0:
            raise RuntimeError(
                f"LoadStates failed: no structures could be loaded from {len(structures)} attempted"
            )

        # Register as a loaded object (use first structure's ID as key)
        self.loaded_objects[structures[0]["id"]] = obj_name
        if self.first_loaded_object is None:
            self.first_loaded_object = obj_name

        # Align all states: to reference if one exists, otherwise to each other
        if self.first_loaded_object and self.first_loaded_object != obj_name:
            # Align each state to the reference object
            print(f"  Aligning {loaded_count} states to '{self.first_loaded_object}'...")
            for state_idx in range(1, loaded_count + 1):
                try:
                    cmd.align(
                        f"{obj_name} and state {state_idx}",
                        self.first_loaded_object
                    )
                except Exception as e:
                    print(f"    State {state_idx}: alignment failed ({e})")
        else:
            # No reference — align all states to state 1
            print(f"  Aligning {loaded_count} states to state 1 (intra_fit)...")
            try:
                cmd.intra_fit(obj_name)
            except Exception as e:
                print(f"    intra_fit failed: {e}")

        print(f"LoadStates: Loaded and aligned {loaded_count} states into '{obj_name}'")
    ...
    def execute_operation(self, op: Dict[str, Any]):
        ...
        elif op_type == "load_states":
            self.execute_load_states(op)
        elif op_type == "color":
